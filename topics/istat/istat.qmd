...
```{r}
source("~/R/OurItalyInData.github.io/topics/functions.R")
...
suicide <- read_db(
  id = "39_1005",
  # dsd = TRUE,
  path = "~/R/OurItalyInData.github.io/data_warehouse/istat/",
  version = "1.0"
  )
...
omicidi_vs_popolazione <- felonies_total %>% 
  ...
  arrange(desc(year), sesso, variable)
```

```{r, to_ojs}
ojs_define(data_r = omicidi_vs_popolazione)
```

```{ojs, ojs_data}
data = transpose(data_r); // Ensure `data_r` is correctly structured

filteredData = data
  .filter(d => selectedSex.includes(d.sesso)) // Filter by selected sex
  .filter(d => d.variable === dropdownVariable); // Filter by selected variable

colorScale = d3.scaleOrdinal()
  .domain([...new Set(data.map(d => d.sesso))])
...
viewof selectedSex = Inputs.checkbox(
  data.map(d => d.sesso),
  {
    value: ["maschi", "femmine"],
    label: "Select Sex",
    unique: true
    }
)

viewof dropdownVariable = Inputs.select(
...
Plot.plot({
  marks: [
    Plot.line(
      ...
      {
        x: "year", 
        y: "value", // Y-axis corresponds to the selected variable's value
        stroke: d => colorScale(d.sesso), // Color based on sex
        strokeWidth: 2
      }
    ),
    Plot.dot(
      filteredData,
      {
        x: "year", 
        y: "value", 
        fill: d => colorScale(d.sesso), // Color based on sex
        tip: d => `Sex: ${d.sesso}, Year: ${d.year}, Value: ${d.value}`
      }
    ),
    Plot.crosshair(filteredData, { x: "year", y: "value" })
  ],
  color: {
    domain: data.map(d => d.sesso),
    range: d3.schemeDark2,
  },
  x: { label: "Year" },
  y: { label: "Value" }
